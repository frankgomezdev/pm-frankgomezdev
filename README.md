...
5. Deploy security rules from this repo (Console → Firestore → Rules, paste `firestore.rules`, Publish), **or** start in test mode briefly for local A2 only. Re-publish whenever `firestore.rules` changes (required after D1 field-validation rules).
